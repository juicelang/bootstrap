...
export default class parser {
	...
	>(
		...
		while (this.cursor < this.tokens.length) {
			...
			if (
				next.kind === "operator" &&
				(next.value === ".." || next.value === "," || next.value === "->" || (is_type_expression && next.value === "="))
			) {
				break;
			}
			...
			if (next.kind === "open_paren") {
				...
				if (last_binary_expression) {
					...
					// @ts-ignore
				} else if (root_sub_expression.kind === "member_expression") {
					const new_root_sub_expression: nodes.function_call_node = {
						kind: "function_call",
						args,
						target: {
			...
			if (next.kind === "macro_args") {
				this.eat_whitespace();
				const p = new parser();
				p.tokens = [
					{ kind: "identifier", value: "f", location: next.location },
					{ kind: "open_paren", location: next.location },
					...next.value,
					{ kind: "close_paren", location: next.location },
				];

				const e = p.parse_expression();

				this.eat();

				if (last_binary_expression) {
					return todo(`parse_expression: macro call for binary expression`);
				} else if (root_sub_expression.kind === "value_expression") {
					const new_root_sub_expression: nodes.macro_call_node = {
						kind: "macro_call",
						location: this.location(root_sub_expression, next),
						// @ts-ignore
						name: root_sub_expression.value,
						// @ts-ignore
						args: e.value.args,
					};

					root_sub_expression = new_root_sub_expression as unknown as nodes.sub_expression_node;
				} else {
					return todo(`parse_expression: macro call for ${root_sub_expression.kind}`);
				}

				const next_non_whitespace = this.peek_non_whitespace();

				if (next_non_whitespace.kind === "macro_body") {
					this.eat_whitespace();
					this.eat();
					// @ts-ignore
					root_sub_expression.body = next_non_whitespace.value;
				}
			}

			if (next.kind === "macro_body") {
				this.eat_whitespace();
				this.eat();

				if (last_binary_expression) {
					return todo(`parse_expression: macro body for binary expression`);
				} else if (root_sub_expression.kind === "value_expression") {
					const new_root_sub_expression: nodes.macro_call_node = {
						kind: "macro_call",
						location: this.location(root_sub_expression, next),
					...
					return todo(`parse_expression: macro body for ${root_sub_expression.kind}`);
				}
			}
	...
	parse_function_node(): nodes.function_node {
		...
		let name: nodes.identifier_node | nodes.macro_identifier_node | null = null;

		if (next.kind === "identifier") {
			name = this.parse_identifier_node();
		} else if (next.kind === "macro_identifier") {
			name = this.parse_macro_identifier_node();
		}

		this.eat_whitespace();

		const open_paren = this.peek();
		if (name && name.kind === "macro_identifier" && open_paren.kind !== "macro_args") {
			throw new Error(`Expected ( after macro function`);
		} else if (open_paren.kind !== "open_paren" && (!name || (name && name.kind !== "macro_identifier"))) {
		...
		if (name && name.kind === "macro_identifier") {
			// @ts-ignore
			args.push(open_paren.value);
		} else {
			while (this.cursor < this.tokens.length) {
				const t = this.peek();
				...
				const arg = this.parse_function_argument_node();
	...
	parse_block_node(): nodes.block_node {
		...
		while (this.cursor < this.tokens.length) {
			...
			if (t.kind === "close_curly" || t.kind === "eof") {
				break;
			}
	...
	parse_tuple_node({
		is_type_expression = false,
	}): nodes.tuple_node | nodes.tuple_type_node {
		const open_paren = this.eat() as tokens.open_paren_token;

		const value: nodes.expression_node[] = [];
		...
		const close_paren = this.eat() as tokens.close_paren_token;

		// @ts-ignore
		return {
			kind: is_type_expression ? "tuple_type" : "tuple",
			value,
